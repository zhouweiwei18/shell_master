...
EOF
		...
		sql_cmd="SELECT CONCAT(CONCAT_WS('|',device1.devicename,table2.NAME,device1.usednum, (case device1.totalnum when '0' then '1' else device1.totalnum end),ROUND((device1.usednum / (case device1.totalnum when '0' then '1' else device1.totalnum end)*100), 2)),'%;') from dms_ipused_stat_device device1, md_area table2 where device1.stattype = 1 AND table2.AREANO = device1.AREANO;"
...
EOF
		...
		# mysql 执行失败返回值是1
		ret_code=$?
		echo "cat read_file start"
		cat read_file
		echo "cat read_file end"

		$(sed -e '/CONCAT_WS/d'  ${read_file}  > ${tmp_file})
		rm ${read_file}
	fi

	echo "cat tmp_file start"
	cat ${tmp_file}
	echo "cat tmp_file end"
  ...
  echo "==================start read tmp_file=================="
  cat data.txt
  echo "==================end read tmp_file=================="


	while read line
	do
	  echo "----------" "$line" "----------"
    # 将变量"line"中最后一个竖线字符（|）后的内容赋值给变量"percent_tmp"
		percent_tmp=${line##*|}
    echo "++++++++++" $percent_tmp "++++++++++"
...
# $2: /home/bmreport/work/zhouww/tools/crygen
# $3:
...
#ipaddr_utilization_bras "sqlplus -S DHCP4_DEV1/0YD9AcIU0KvFE+fdADbTSA==@10.21.17.145:1521/LCDMP3" "/home/bmreport/work/zhouww/tools/crygen" ""
